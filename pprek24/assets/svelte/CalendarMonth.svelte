<script lang="ts">
  ...
  const onResize = $derived.by<ResizeObserverCallback>(() => (entries) => {
    entryLoop: for (const entry of entries) {
      const headerCells = Array.from(
        entry.target.querySelectorAll("thead tr th"),
      ) as HTMLTableCellElement[];
      const padding = getComputedStyle(headerCells[0])
        .paddingInline.split(" ")
        .reduce((acc, cur) => acc + parseFloat(cur), 0);
      const maxWidth = headerCells[0].getBoundingClientRect().width - padding;

      for (const cell of headerCells) {
        const longText = cell.querySelector(
          ".pprek-calendar-weekday--long",
        ) as HTMLSpanElement;
    ...
    fetch(` ${config.calendar_api_url}/${year}/${month}`, { cache: "default" })
      .then(async (res) => {
        if (
          res.status !== 200 &&
          res.status !== 304 &&
          res.headers.get("X-API-Version") !== "2"
        ) {
      ...
      .then((res) => {
        const days: Month = new Array(6)
          .fill(null)
          .map(() => new Array(7).fill(null)) as any;
        ...
        for (let w = 0; w < 6; w++) {
          for (let d = 0; d < 7; d++) {
            ...
            days[w][d] = {
              ...
              position:
                date.getMonth() + 1 === month ? "in-month" : "out-of-month",
            };
            date.setHours(24);
          }
        }
...
<div aria-live="off" aria-atomic="true" aria-busy={busy}>
  ...
  {:then { days }}
    <header class="pprek-calendar-month-title">
      <button class="pprek-calendar-month-prev" onclick={prev}
        ><Icon icon="caretLeftFill" aria-hidden="true" /></button
      >
      <h2 id="{id}_title">
        Unsere Termine im {dateFormatters.longMonthFormatter.format(
          firstOfMonth,
        )}
      </h2>
      <button class="pprek-calendar-month-next" onclick={next}
        ><Icon icon="caretRightFill" aria-hidden="true" /></button
      >
    </header>
    <table
      class="pprek-calendar-month"
      {id}
      role="grid"
      ...
      <thead aria-hidden="true">
        <tr>
          {#snippet th(day: 1 | 2 | 3 | 4 | 5 | 6 | 7)}
            ...
            <th>
              <span
                class="pprek-calendar-weekday--long"
                class:sr-only-modified={!longTitles}
                >{dateFormatters.longWeekDayFormatter.format(date)}</span
              >
              <span
                class="pprek-calendar-weekday--short"
                aria-hidden="true"
                style:display={longTitles ? "none" : undefined}
                >{dateFormatters.shortWeekDayFormatter.format(date)}</span
              >
            </th>
          {/snippet}
          {@render th(1)}
        ...
        {#each days as week}
          <tr>
            {#each week as day}
              <td
                role="gridcell"
                aria-disabled={day.position === "out-of-month"
                  ? true
                  : undefined}
                        ...
                        <a href="{config.calendar_page}?{query.toString()}">
                          {#if !event.allDay && event.start.includes(day.dateString)}
                            <time datetime={event.start}
                              >{dateFormatters.shortTimeFormatter.format(
  ...
  .pprek-calendar-month {
    ...
    td {
      ...
      ul {
        ...
        gap: 0.25rem;
        overflow: auto;
        max-block-size: calc(100% - 1.5rem - 0.25rem - 0.25rem);
      }
